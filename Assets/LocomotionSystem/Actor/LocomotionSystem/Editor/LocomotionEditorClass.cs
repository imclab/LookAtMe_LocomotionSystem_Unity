...
class InspectorAnimationGroup {
	public string name;
    public bool fixAllFootSkating = false;
		...
		if (animsFoldout) {
			...
			for (int g=1; g<groups.Count; g++) 
			{
				...
				if (group.expanded) 
				{
					...
					// Do fix all Foot Skating if needed.
					if ("Not Used" != group.name)
					{
						GUI.changed = false;
						group.fixAllFootSkating = EditorGUILayout.Toggle("Fix all Foot Skating", group.fixAllFootSkating);
						if (GUI.changed)
						{
							foreach (MotionAnalyzer ma in group.motions)
							{
								ma.fixFootSkating = group.fixAllFootSkating;
							}
							changed = true;
						}
					}
					...
					for (int m=0; m<group.motions.Count; m++) {
						...
						if (used) {
							...
							GUI.changed = false;
							ma.animation = EditorGUILayout.ObjectField(ma.animation, typeof(AnimationClip), true, GUILayout.ExpandWidth(true)) as AnimationClip;
							...
							EditorGUILayout.ObjectField(ma.animation, typeof(AnimationClip), true, GUILayout.ExpandWidth(true));
							GUI.enabled = true;
