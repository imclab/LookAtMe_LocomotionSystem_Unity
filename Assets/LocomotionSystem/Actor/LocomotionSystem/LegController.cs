...
public class LegController : MonoBehaviour {
	...
	public float MinAdvisedSpeed { get; private set; }
	public float MaxAdvisedSpeed { get; private set; }
	...
	private void computeMinAndMaxAdvisedSpeed()
	{
		MotionGroupInfo locomotionGroup = null;
		foreach (MotionGroupInfo mg in motionGroups)
		{
			if (mg.name == "locomotion")
			{
				locomotionGroup = mg;
			}
		}
		if (null == locomotionGroup)
		{
			Debug.LogError("No MotionGroup named locomotion found !");
			return;
		}
		MinAdvisedSpeed = float.MaxValue;
		MaxAdvisedSpeed = float.MinValue;
		foreach (IMotionAnalyzer ma in locomotionGroup.motions)
		{
			float maSpeed = ma.cycleSpeed * transform.localScale.x;
			if (maSpeed < MinAdvisedSpeed)
			{
				MinAdvisedSpeed = maSpeed;
			}

			if (maSpeed > MaxAdvisedSpeed)
			{
				MaxAdvisedSpeed = maSpeed;
			}
		}
		// min advised speed must be >= 1.0f, for LocomotionSystem to work good.
		if (MinAdvisedSpeed < 1.0f)
		{
			MinAdvisedSpeed = 1.0f;
		}
	}
	...
	public void Init()
	{
		// Init ARMS CONTROLLERS
		/*
		IKSolverRestrictionLeft LeftArm = GetComponent<IKSolverRestrictionLeft>() as IKSolverRestrictionLeft;
		IKSolverRestrictionRight RightArm = GetComponent<IKSolverRestrictionRight>() as IKSolverRestrictionRight;
		LeftArm.InitLeftArm();
		RightArm.InitRightArm();
		*/
 
		//Debug.Log(this.name);
		// Autocreate legs for RocketBox Characters.
		// Do not do this here. Because there are not only RocketBox characters.
		//rootBone = transform.FindChild("Bip01");
		//legs[0].hip = transform.FindChild("Bip01/Bip01 Pelvis/Bip01 Spine/Bip01 L Thigh");
		//legs[0].ankle = transform.FindChild("Bip01/Bip01 Pelvis/Bip01 Spine/Bip01 L Thigh/Bip01 L Calf/Bip01 L Foot");
		//legs[0].toe = transform.FindChild("Bip01/Bip01 Pelvis/Bip01 Spine/Bip01 L Thigh/Bip01 L Calf/Bip01 L Foot/Bip01 L Toe0/Bip01 L Toe0Nub");

		//legs[1].hip = transform.FindChild("Bip01/Bip01 Pelvis/Bip01 Spine/Bip01 R Thigh");
		//legs[1].ankle = transform.FindChild("Bip01/Bip01 Pelvis/Bip01 Spine/Bip01 R Thigh/Bip01 R Calf/Bip01 R Foot");
		//legs[1].toe = transform.FindChild("Bip01/Bip01 Pelvis/Bip01 Spine/Bip01 R Thigh/Bip01 R Calf/Bip01 R Foot/Bip01 R Toe0/Bip01 R Toe0Nub");
	...
	void Awake() {
		...
		// compute min and max advised speeds.
		computeMinAndMaxAdvisedSpeed();
	}
