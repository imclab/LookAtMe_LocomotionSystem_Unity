...
public class LegAnimator : MonoBehaviour 
{
	...
	public bool CorrectionOn;
	public Vector3 correction;
	public Vector3 _SavedCorrection;
	...
	private IKSolver ikSolver = null;
	private IK1JointAnalytic _MyIKJointAnalitic = new IK1JointAnalytic();
	private IKSimple _MyIKSimple = new IKSimple();
	...
	void Start () 
	{
		...
		basePointFoot = new Vector3[legs.Length];
		...
		OldPoint = NewPoint = legC.root.transform.position;
	}
	...
	public Vector3 GetLeftAnklePosition()
	{
		for (int leg=0; leg<legs.Length; ++leg)
		{
			if(legs[leg].ankle.gameObject.name.Contains(" L "))
			{
				return legs[leg].ankle.position;
			}
		}
		return new Vector3();
	}
	public Vector3 GetRightAnklePosition()
	{
		for (int leg=0; leg<legs.Length; ++leg)
		{
			if(legs[leg].ankle.gameObject.name.Contains(" R "))
			{
				return legs[leg].ankle.position;
			}
		}
	...
	public void EnableCorrection(bool is_ascending)
	{
		if(is_ascending)
		{
			CorrectionOn = true;
			correction = _SavedCorrection;
		}
		else
			DisableCorrection();
	}
	...
	void LateUpdate () 
	{
		...
		// jjh optim : don't use ik if velocity is null
		//useIK = (tr.velocity.sqrMagnitude>0.01f);
		...
		NewPoint = legC.root.transform.position;
		CurrentVelocity = NewPoint-OldPoint;
		OldPoint = NewPoint;
		...
		if (!useIK)
		{
			...
			//{
			//	transform.position = new Vector3(position.x, Terrain.activeTerrain.SampleHeight(position), position.z);
			//}
			return;
		}
		...
		//this.transform.position = new Vector3(transform.position.x, transform.position.y+ val, transform.position.z);
		
		//this.transform.position = new Vector3(transform.position.x, transform.position.y+ 0.01f, transform.position.z);
		this.transform.position = this.transform.position+correction;
		//Profiler.BeginSample("Leganimator0");
		...
		for (int leg=0; leg<legs.Length; leg++) 
		{
			...
			if (legStates[leg].cycleTime < legStates[leg].strikeTime) 
			{
				...
				else 
				{
					...
					float maxRotateAngle = rotationSeekSpeed / flightTime * ttime;
				...
				else 
				{
					...
					if (moveVector!=Vector3.zero && predictedStrikeTime>0) 
					{
						...
						float moveDist = Mathf.Min(
							moveVectorMag,
							Mathf.Max(
							stepSeekSpeed / Mathf.Max(0.1f,flightTime) * ttime,
								(1+2*Mathf.Pow(towardStrike-1,2))
							* (ttime/predictedStrikeTime)
									* moveVectorMag
							)
						);
		...
		//Profiler.EndSample();
		//Profiler.BeginSample("Leganimator1");
		...
		//Profiler.EndSample();
		//Profiler.BeginSample("Leganimator2");
		...
		//Profiler.EndSample();
		//Profiler.BeginSample("Leganimator3");
		if (
		...
		//Profiler.EndSample();
		//Profiler.BeginSample("Leganimator4");
		...
		//Profiler.EndSample();
		//Profiler.BeginSample("Leganimator5");
		...
		//Profiler.EndSample();
		//Profiler.BeginSample("Leganimator6");
		...
		//Profiler.EndSample();
		//Profiler.BeginSample("Leganimator7");
		for (int leg=0; leg<legs.Length; leg++) {
			// Value from 0.0 at liftoff time to 1.0 at strike time
			float flightTime = Mathf.InverseLerp(
				legStates[leg].liftoffTime, legStates[leg].strikeTime, legStates[leg].cycleTime);
		...
		//Profiler.EndSample();
		//Profiler.BeginSample("Leganimator8");
		...
		//Profiler.EndSample();
		//Profiler.BeginSample("Leganimator9");
		// Apply body rotation
		...
		legC.root.transform.position = rootPointAdjusted + position-transform.position+correction;
		
		//Profiler.EndSample();
		//Profiler.BeginSample("Leganimator10");
		for (int leg=0; leg<legs.Length; leg++) {
			legStates[leg].hipReference = legs[leg].hip.position;
			legStates[leg].ankleReference = legs[leg].ankle.position;
		}
		//Profiler.EndSample();
		//Profiler.BeginSample("Leganimator10.0");
		...
		{
			//Profiler.BeginSample("Leganimator10.1");
			...
			//Profiler.EndSample();
			//Profiler.BeginSample("Leganimator10.2");
			
			// Find and apply the hip offset
			FindHipOffset();
			//Profiler.EndSample();
			//Profiler.BeginSample("Leganimator10.3");

			// Adjust the legs according to the found ankle and hip positions
			for (int leg=0; leg<legs.Length; ++leg)
			{ 
				AdjustLeg(leg, legStates[leg].ankle, pass==2); 
			}
			//Profiler.EndSample();
		}
		
		//Profiler.EndSample();
		//Profiler.BeginSample("Leganimator11.0");
		for (int leg=0; leg<legs.Length; leg++) {
		...
		//Profiler.EndSample();
		//Profiler.BeginSample("Leganimator11");
	...
	public Matrix4x4 FindGroundedBase(
		...
		bool hitB = false;
		bool valid = false;
		//Profiler.BeginSample("physique1");
		if (Physics.Raycast( pos + up*maxStepHeight,
			-up,
			out hit,
			maxStepHeight*2,
			groundLayers)) 
		...
		//Profiler.BeginSample("physique2");
		if (Physics.Raycast( pos + up*maxStepHeight + heelToToetip,
			-up,
			out hit,
			maxStepHeight*2,
			groundLayers))
	...
	public void FindHipOffset() 
	{
		...
		float intersectionsX;
		float intersectionsY;
		for (int leg=0; leg<legs.Length; leg++) 
		{
			intersectionsX = float.NaN;
			intersectionsY = float.NaN;
			...
			Util.GetLineSphereIntersections(
				//Vector3.zero,
				ref legsUp,
			...
			if (!float.IsNaN(intersectionsY)) 
			{
				hipDesiredHeight = intersectionsY;
	...
	public void AdjustLeg(int leg, Vector3 desiredAnklePosition, bool secondPass) 
	{
		...
		{
			ikSolver = _MyIKJointAnalitic;
		}
		else
		{
			ikSolver = _MyIKSimple;
		}
