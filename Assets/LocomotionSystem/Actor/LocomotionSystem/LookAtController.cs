...
public class LookAtController : MonoBehaviour
{
	...
	private Vector3[] jointDirections;
	
	public void InitSegments() 
	{
		segments = new BendingSegment[2];
		segments.SetValue(new BendingSegment(), 0);
		segments.SetValue(new BendingSegment(), 1);
		// On initialise la liste des joints nécessaires au fonctionnement du lookat (spécifique RocketBox => pas générique).
		segments[0].firstTransform = transform.FindChild("Bip01/Bip01 Pelvis/Bip01 Spine/Bip01 Spine1");
		segments[0].lastTransform = transform.FindChild("Bip01/Bip01 Pelvis/Bip01 Spine/Bip01 Spine1/Bip01 Spine2");
		segments[1].firstTransform = transform.FindChild("Bip01/Bip01 Pelvis/Bip01 Spine/Bip01 Spine1/Bip01 Spine2/Bip01 Neck");
		segments[1].lastTransform = transform.FindChild("Bip01/Bip01 Pelvis/Bip01 Spine/Bip01 Spine1/Bip01 Spine2/Bip01 Neck/Bip01 Head");

		nonAffectedJoints = new NonAffectedJoints[2];
		nonAffectedJoints.SetValue(new NonAffectedJoints(), 0);
		nonAffectedJoints.SetValue(new NonAffectedJoints(), 1);
		nonAffectedJoints[0].joint = transform.FindChild("Bip01/Bip01 Pelvis/Bip01 Spine/Bip01 Spine1/Bip01 Spine2/Bip01 Neck/Bip01 L Clavicle/Bip01 L UpperArm");
		nonAffectedJoints[1].joint = transform.FindChild("Bip01/Bip01 Pelvis/Bip01 Spine/Bip01 Spine1/Bip01 Spine2/Bip01 Neck/Bip01 R Clavicle/Bip01 R UpperArm");
	}
	void Start () 
	{
		InitSegments();

		//rootNode = GameObject.Find(this.name+"/Bip01").gameObject.transform;		
		if (rootNode == null) 
		{
			rootNode = transform;
		}
	...
	void LateUpdate () 
	{
		...
		for (int i=0; i<nonAffectedJoints.Length; ++i) 
		{
			...
			{
				jointDirections[i] = nonAffectedJoints[i].joint.GetChild(0).position - nonAffectedJoints[i].joint.position;
			}
			
			//foreach (Transform child in MyaffectedJoint.joint) 
			//{
		...
		foreach (BendingSegment segment in segments) 
		{
			...
			if (manual)
			{
				if(target != null) // Desired look direction in world space
				{
					lookDirWorld = (target.position - segment.lastTransform.position).normalized;
				}
				else
				{
					lookDirWorld = Vector3.zero;
				}
			}
			else
			{
				// Desired look direction in world space
				lookDirWorld = transform.rotation * headLookVector * 0.01f;
		...
		for (int i=0; i<nonAffectedJoints.Length; i++) 
		{
			...
			if(nonAffectedJoints[i].joint.childCount>0)
			{
				newJointDirection = nonAffectedJoints[i].joint.GetChild(0).position - nonAffectedJoints[i].joint.position;
			}
			/*int uu = 0;
			foreach (Transform child in nonAffectedJoints[i].joint.childCount) 
			{
				++uu;
