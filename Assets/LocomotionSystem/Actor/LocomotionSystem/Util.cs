...
public class Util {
	...
	// version julian avec linestart =(0,0,0)
	public static void GetLineSphereIntersections(ref Vector3 lineDir, Vector3 sphereCenter, float sphereRadius, out float retX, out float retY) 
	{		
		Profiler.BeginSample ("GetLineSphereIntersections");
		float a = lineDir.sqrMagnitude;
		float b = 2 * (-Vector3.Dot(lineDir, sphereCenter));
		float c = sphereCenter.sqrMagnitude - sphereRadius*sphereRadius;
		float d = b*b - 4*a*c;
		if (d<0)
		{
			retX = float.NaN;
	...
	/*public static float[] GetLineSphereIntersections(ref Vector3 lineStart, ref Vector3 lineDir, ref Vector3 sphereCenter, ref float sphereRadius) 
	{
