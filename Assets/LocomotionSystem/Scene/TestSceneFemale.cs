...
public class TestActorListener : ActorListener
{
	MotionsController _MotionsController = null;
	
	public TestActorListener(MotionsController motions_controller)
	{
		_MotionsController = motions_controller;
	}
	public void MotionIsDone(string motion_id)
	{
		Debug.Log("Motion is done : "+ motion_id);
		_MotionsController.DeleteMotion(motion_id);
	}
}
	
public class TestSceneFemale : MonoBehaviour {
	...
	void Update () 
	{
		...
		{
			Init();
			_MotionsController.PlayMotion("f_dancing_sexy", false);
			_EmotionController.PlayEmotion("SMILE_03");
			_SpeakController.Say("I will take you anywhere. But what will you do? Run away? Confront him?", "");
