...
class FollowTrajectoryEditor : Editor 
{	
	...
	private static GUIContent OpenFileGuiBtn = new GUIContent("Find", "Open a file editor");
	private static GUILayoutOption buttonWidth = GUILayout.MaxWidth(33f);
	...
	public override void OnInspectorGUI () 
	{
		...
		EditorGUILayout.PropertyField(_TrajectroryFilenameProperty, GUILayout.ExpandWidth(true));
		if(GUILayout.Button(OpenFileGuiBtn, EditorStyles.miniButtonRight, buttonWidth))
		{
			string res = EditorUtility.OpenFilePanel("Select a trajectory file","Assets","csv");
			int ii = res.IndexOf("Assets");
			res = res.Substring(ii,res.Length-ii);
			_TrajectroryFilenameProperty.stringValue = res;
		}
